...
(function() {
  ...
  is_script_tag = function(token) {
    return token === '<script';
  };

  ends_in_end_script_tag = function(token) {
    var token_end;
    token_end = token.substr(token.length - 9);
    return token_end === '</script>';
  };
  ...
  return_dual_pane = function(before, after) {
    return {
      before: before,
      after: after
    };
  ...
  render_operations_dual_pane = function(before_tokens, after_tokens, operations) {
    var after_render, before_render, i, len, next_block, op;
    before_render = '';
    after_render = '';
    for (i = 0, len = operations.length; i < len; i++) {
      op = operations[i];
  ...
  diff_dual_pane = function(before, after) {
    var ops;
    if (before === after) {
